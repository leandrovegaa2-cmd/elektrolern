    ...
    <>
      <div className="topbar">
        ...
        <span className="counter session-counter">
          <small>QUIZ</small> {String(index + 1).padStart(2, "0")} / {String(fragen.length).padStart(2, "0")} · {punkte} ✓
        </span>
      </div>
        ...
        {beantwortet ? (
          <>
            ...
            <button className="next-btn" onClick={() => setIndex((i) => i + 1)}>
              <span>{optionen[gewaehlt]?.richtig ? "Richtig! Weiter →" : "Weiter →"}</span><i aria-hidden="true">→</i>
            </button>
          </>
        ) : null}
      </div>
