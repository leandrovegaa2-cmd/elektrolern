...
export default function ProgressScreen({ progress, sync, onProblemkarten, onFehlerheft, onLernfeldUeben, onEditor, onTabWechsel }) {
  ...
  return (
    <>
      ...
      <section className="screen-intro progress-intro">
        <span className="screen-code">LEISTUNG / STATUS</span>
        <h1>Dein Fortschritt</h1>
        <p>{gesamt} Karten insgesamt · {neu} noch nie gesehen{eigene > 0 ? " · " + eigene + " davon eigene" : ""}</p>
        <div className="progress-grade">
          <span>Gesamtfortschritt</span>
          <b>{Math.round((gelernt / gesamt) * 100)}<small>%</small></b>
          <i><em style={{ transform: `scaleX(${gelernt / gesamt})` }} /></i>
        </div>
      </section>
      ...
      <div className="stat-kacheln">
        ...
        <div className="stat-k">
          <div className="sz"><span className="stat-signal">◈</span> {progress.state.streak.count}</div>
